...
(use-package org-agenda
  ...
  ((org-agenda-files (list org-directory (concat org-directory "journal")))
   ...
   (org-agenda-clockreport-parameter-plist
    (quote (:link t :maxlevel 9 :fileskip0 t :compact t :narrow 120)))))
#+end_src
...
(use-package org-capture :defer t
  :after org
  :preface
  (defun afsmnghr/org-journal-file ()
    "Return the path to the journal file for the current year."
    (expand-file-name (format "%s.org" (format-time-string "%Y"))
                      (expand-file-name "journal" org-directory)))

  (defconst afsmnghr/org-capture-templates
    '(("j" "Journal" entry (file+olp+datetree afsmnghr/org-journal-file)
       "* %?" :jump-to-captured t)))
  :custom
